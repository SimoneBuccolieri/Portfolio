<template>
  <div id="app" class="container flex justify-center items-center bg-white overflow-hidden mx-auto ">
    <div v-if="playBtn" class="flex flex-col justify-center">
      <h1 class="flex justify-center font-bold">Snake</h1>
      <button @click="play" class="mt-5 h-10 bg-white border border-gray-200 hover:bg-blue-600 hover:text-blue-100 border-solid rounded-sm px-5">Play</button>
    </div>
    <div>
...
export default {
  data() {
    return {
      ...
      playBtn: true,
      gameOver: false,
      canvawidth: window.innerWidth * 0.5, // x
      canvaheight: window.innerHeight * 0.5, // y
      snake: {
        x: [100],
        y: [100],
        radius: 8,
